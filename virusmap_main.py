...
def mainProgess(virusPlaceListInput):
    ...
    # 写入地址
    virusPlaceListOutShanghai = []
    virusPlaceListOut = np.unique(sum(sum(virusPlaceList, []), []))
    for i in virusPlaceListOut :
        virusPlaceListOutShanghai.append('上海市'+i)
    print('outputStart' + str(virusPlaceListOutShanghai))
    sheet1.range('a' + placeStart).options(transpose=True).value = virusPlaceListOutShanghai

    # 查找经纬度并写入
    viruslocationList = []
    for address in virusPlaceListOutShanghai:
